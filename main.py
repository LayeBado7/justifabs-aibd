...
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///./aibd_absences.db")
...
def seed():
    with SessionLocal() as db:
        if db.scalar(select(func.count(User.id))) == 0:
            users = [
                User(matricule="AIBD001", full_name="Agent Démonstration", email="agent@aibd.sn", department="Systèmes d'information", site="DIASS", role="agent", password_hash=pwd_context.hash("Aibd@2026")),
                User(matricule="RH001", full_name="Gestionnaire RH", email="rh@aibd.sn", department="Ressources Humaines", site="DIASS", role="rh", password_hash=pwd_context.hash("Aibd@2026")),
                User(matricule="ADM001", full_name="Administrateur", email="admin@aibd.sn", department="Systèmes d'information", site="DIASS", role="admin", password_hash=pwd_context.hash("Aibd@2026")),
            ]
            db.add_all(users)
            db.commit()
seed()
